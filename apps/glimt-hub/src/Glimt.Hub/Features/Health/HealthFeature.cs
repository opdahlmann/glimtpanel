...
public sealed record HealthResponse(string Status, string Version, string Env, string Mongo, int AgentsConnected, long UptimeSec);
...
public static class HealthFeature
{
    ...
    {
        app.MapGet(Path, (GlimtOptions options, MongoContext mongo, AgentRegistry registry) => Results.Ok(new HealthResponse(
            "ok",
            Version,
            options.Env,
